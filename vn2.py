...
class AdvancedProxyChecker:
    def __init__(self, config, exporter, type_filter=None, speed_filter=None):
        ...
        self.check_urls = ['https://vnexpress.net', 'https://google.com.vn', 'https://www.google.com.vn']
    ...
    def _perform_check(self, proxy_info):
        try:
            host, port_str = proxy_info.address.split(':')
            port = int(port_str)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(5)
            sock.connect((host, port))
...
class VNIPGenerator:
    ...
    def _get_default_port(self, ptype):
        ports = {
            'http': [8080, 3128, 1001, 1002],
            'https': [8080, 3128, 1001, 1002],
            'socks4': [1080, 1001, 1002],
            'socks5': [1080, 1001, 1002],
        }
        p_list = ports.get(ptype.lower(), [8080])
...
class TelegramBotController:
    def __init__(self, token, allowed_users=None):
        ...
        self.fetcher = None
        self.exporter = None
        self.config = {'threads': 50, 'timeout': 10, 'check_times': 2, 'batch': 5000}
    ...
    async def start(self):
        ...
        commands = [
            ...
            BotCommand("of", "Tắt và xóa tin nhắn status")
        ]
    ...
    async def cmd_scan(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(f"Dang tim {quantity} proxy loại {proxy_type.upper()} - Tốc độ: {speed_filter or 'tất cả'} đợi đi...")
    ...
    async def start_scan(self, update, proxy_type, speed_filter, quantity):
        try:
            ...
            await update.message.reply_text(f"ok {len(batched)} đang bắt đầu...")
    ...
    def _get_modern_status(self):
        if not self.scanner:
            return "Chưa có scan nào."
        ...
        status = (
            "Status\n"
            "------------------------\n"
            f"Checked     : {checked}/{total} ({progress}%)\n"
            f"Valid VN    : {valid}\n"
            f"Fast        : {fast}\n"
            f"Elite       : {elite}\n"
            f"Scanning    : {'Dang chay' if self.is_scanning else 'Da dung'}\n"
            "lâu vcl"
        )
        return status

    async def _status_update_loop(self):
        while self.is_scanning and self.status_message is not None:
            await asyncio.sleep(5)
            if self.scanner and self.status_message:
                status_text = self._get_modern_status()
                try:
    ...
    async def cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.scanner:
            ...
            return
        status_text = self._get_modern_status()
        self.status_message = await update.message.reply_text(status_text)
        if self.is_scanning and self.status_update_task is None:
    ...
    async def cmd_of(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.status_message:
            ...
            await update.message.reply_text("chưa bật status")
    ...
    async def cmd_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        help_text = (
            ...
            "/status              → status\n"
            "/of                  → Tắt & xóa tin nhắn status\n"
            "/st ...              → cài đặt threads, timeout, check_times...\n"
...
{C.N}"""
    print(banner)
    print(f"{C.G}[MODE]{C.N} moc mat may ra bay gio")
...
async def main():
    ...
    await bot.start()
    print(f"{C.G}[SYSTEM]{C.N} -Nhấn Ctrl+C để dừng")
